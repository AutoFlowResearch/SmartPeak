...
# bash script to further customize the SmartPeak dmg
...
DISK_NAME=SmartPeak-2.3.0-Darwin
DMG_NAME=${DISK_NAME}.dmg
SMARTPEAK_NAME=SmartPeak-2.3.0
    ...
    with timeout of 300 seconds
      ...
      -- if we have a fixed resolution we can also set this
    	set icon size of theViewOptions to 72
  		set bgimg to "'${SMARTPEAK_NAME}':share:SmartPeak:background.png" as text
  		set background picture of theViewOptions to file bgimg

  		set the position of item "'${SMARTPEAK_NAME}'" of container window to {470, 140}
